		...
		return (
			<div className={cn("relative h-full", className)} ref={containerRef}>
				<TextareaAutosize
					...
					minRows={1}
					{...Object.fromEntries(Object.entries(props).filter(([key]) => !["onFilePasted", "style"].includes(key)))}
				/>
				...
				<pre
					...
					className={cn(
						...
						"no-scrollbar pointer-events-none absolute inset-0 m-0 block w-full bg-transparent font-[inherit] text-black",
					)}
